...
| "no history of substance abuse" / "no history of addiction" / excluding applicants for a past addiction | A past history of drug or alcohol addiction, including being in recovery, is itself a protected disability under the FHA. Excluding someone for it is a handicap-based limitation, the same as excluding someone for a physical disability. | National Housing Law Project, "Fair Housing and Reentry" (nhlp.org, retrieved 2026-09-05): "Individuals suffering from drug addiction who have successfully completed some form of rehabilitation program are considered disabled on the basis of their recorded history of addiction..." |

**The boundary this one turns on: history is protected, current illegal use is not.** A past addiction, or being in recovery, is a protected disability. **Current, ongoing illegal drug use is explicitly not protected** under the FHA (same source: "current use of illegal substances cannot constitute a disability under the FHA"). So "no current illegal drug use" or "must not be actively using illegal drugs" is not a disability-based limitation and doesn't get flagged on that basis. It's the *history* framing ("no history of," "never had a problem with," excluding someone for a past addiction) that indicates a preference against a protected disability. Alcoholism follows the same history/current-use logic; this table doesn't repeat every phrasing variant, but the boundary is the same one.
...
Multiple sources agree these describe the property, not the occupant, and are not violations on their own: "master bedroom" / "master suite," "family room," "walk-in closets," "walk to schools" / "walk to bus stop," "kosher meals served on premises" (describes a service, not a requirement), "Merry Christmas" / "Happy Easter" (holiday greeting, not an occupancy condition), "non-smoking," "accessible housing," "senior housing" (when the property qualifies for the Housing for Older Persons Act exemption; see the carve-out below, this "when it qualifies" is doing real work, not a formality). Source: Fair Housing Institute, MetroTex (both retrieved 2026-09-03).

**"Senior housing" is the acceptable case; "55 and over" as an occupancy rule needs the HOPA check, not an automatic pass.** The "Explicitly acceptable" entry above assumes the property genuinely qualifies for the exemption. An ad that states an age floor on who may live there ("55 and over," "adults only," "no residents under 18") is not automatically the acceptable case. It's a familial-status matter that's lawful only if HOPA qualification actually holds, and an ad's text alone never proves that. See `rules.md` rule 5b for the verdict, severity, and the note this requires. Don't let "senior housing" being clean read across to every age-floor phrase being clean too.
